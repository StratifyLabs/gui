...
public:
  ...
  static void setup(Generic container) {
    container.add(Github(Data::create(Names::github_object))).add_event_callback(EventCode::entered, entered);
  }

  //Workers are used to run tasks in the background and then update the graphics
  //in the main thread
  class UpdateWorker : public design::WorkerAccess<UpdateWorker> {
  public:
    UpdateWorker() = default;
    UpdateWorker(lvgl::Runtime *runtime) : design::WorkerAccess<UpdateWorker>(runtime) {}
